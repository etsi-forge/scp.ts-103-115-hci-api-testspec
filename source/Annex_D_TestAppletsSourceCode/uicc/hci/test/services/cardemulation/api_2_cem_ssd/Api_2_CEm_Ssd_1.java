 ...
 * </code>
 */
public class Api_2_CEm_Ssd_1 extends Applet implements CardEmulationListener {

	/*
	 * Define specific SWs
	 */

	private final static short SW_EVENT_CONDITIONS_HCI_DISABLED = ISO7816.SW_NO_ERROR + (short) 1;
	private final static short SW_EVENT_CONDITIONS_FRAGMENTED_MESSAGE = ISO7816.SW_NO_ERROR + (short) 2;
	private final static short SW_EVENT_CONDITIONS_NO_RESOURCES = ISO7816.SW_NO_ERROR + (short) 3;
	private final static short SW_METHOD_NOT_SUPPORTED = ISO7816.SW_UNKNOWN + (short) 2;
	...
	public void process(APDU apdu) throws ISOException {
		...
		case INS_CHECK_FOR_EXCEPTION:
			if ((exceptions[0] & 0x01) == 0x01)
				ISOException.throwIt(SW_EVENT_CONDITIONS_HCI_DISABLED);
			else if ((exceptions[0] & 0x02) == 0x02)
				ISOException.throwIt(SW_EVENT_CONDITIONS_FRAGMENTED_MESSAGE);
			else if ((exceptions[0] & 0x04) == 0x04)
				ISOException.throwIt(SW_EVENT_CONDITIONS_NO_RESOURCES);
			else if ((exceptions[0] & 0x80) == 0x80)
